...
impl OnnxLibraryFixer {
    ...
    pub fn auto_fix() -> Result<bool> {
        ...
        println!("{}", "⚠ ONNX Runtime library not found in binary directory".yellow());
    ...
    fn copy_library(source: &Path, target: &Path) -> Result<bool> {
        ...
        fs::copy(source, target)
            .context("Failed to copy library")?;
    ...
    fn show_manual_fix_instructions() {
        println!();
        println!("{}", "❌ Could not automatically fix the ONNX library issue.".red());
        println!();
        ...
        #[cfg(target_os = "windows")]
        {
            println!("  If you installed via cargo, run:");
            println!("{}", "  > cargo build --release".cyan());
            println!("{}", "  > copy target\\release\\onnxruntime.dll %USERPROFILE%\\.cargo\\bin\\".cyan());
            println!();
            println!("  Or in PowerShell:");
            println!("{}", "  PS> cargo build --release".cyan());
            println!("{}", "  PS> Copy-Item target\\release\\onnxruntime.dll $env:USERPROFILE\\.cargo\\bin\\".cyan());
        }
